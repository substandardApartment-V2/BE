-- 인코딩 설정 및 데이터베이스 생성
SET NAMES utf8mb4;
CREATE DATABASE IF NOT EXISTS my_apt CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE my_apt;

-- 외래 키 비활성화 (데이터 삽입 중 제약 조건 오류 방지)
SET FOREIGN_KEY_CHECKS = 0;

-- 1. 테이블 생성
-- 아파트 테이블
CREATE TABLE apts (
                      apts_id             VARCHAR(255) NOT NULL PRIMARY KEY,
...
CREATE TABLE defect_apts (
                             defect_apts_id BIGINT AUTO_INCREMENT PRIMARY KEY,
                             ...
                             CONSTRAINT FK8a4knpfwna8rdnimwn9mu9d4a FOREIGN KEY (apts_id) REFERENCES apts (apts_id)
);

-- 아파트 상세 테이블
CREATE TABLE detail_apts (
                             detail_apts_id                               VARCHAR(255) NOT NULL PRIMARY KEY,
                             ...
                             apts_id                                      VARCHAR(255) NULL,
                             CONSTRAINT FK896syuqrqc6njrcpsy7kdiksn FOREIGN KEY (apts_id) REFERENCES apts (apts_id)
);

-- 관리비 상세 테이블
CREATE TABLE mng_cost (
                          mngcost_id                          VARCHAR(255) NOT NULL PRIMARY KEY,
                          ...
                          CONSTRAINT FKdgyt3i6et1gw710nrjr7nb40l FOREIGN KEY (detail_apts_id) REFERENCES detail_apts (detail_apts_id)
);
...
CREATE TABLE notices (
                         notices_id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
-- 2. 데이터 삽입 (의존성 순서: apts -> defect_apts -> detail_apts -> mng_cost -> news -> notices)
-- 아파트 기본정보
INSERT INTO apts (apts_id, apt_nm, rdnmadr, nmhsh, use_aprv_year, lo, la, cnst_entrprs_nm, hus_mngm_entrprs_nm, buld_stru, is_defect)
VALUES ('APT1111030000080230000001', '경희궁 롯데캐슬아파트', '철근콘크리트구조', 195, 2019, 126.959518033, 37.5737761339, '롯데건설', '세화종합관리(주)', '철근콘크리트', 0),
       ('APT1168048624120011000001', '디아크리온강남', '서울특별시 강남구 자곡로 11길 11', 597, 2023, 127.1071361532, 37.4767934538, '양우종합건설', NULL, '무량판', 1),
       ('APT3315032510130550003901', '공주월송엘에이치천년나무4단지', '충청남도 공주시 무령로 550-39', 374, 2022, 127.1514146184, 36.4723878921, NULL, NULL, '무량판', 1),
       ('APT4148032061370050000001', '후곡마을뜨란채주공4단지', '경기도 파주시 후곡로 50', 1638, 2004, 126.771139829, 37.7532464307, '대한주택공사', '대원종합관리(주)', '철근콘크리트', 0);

-- 부실 아파트
INSERT INTO defect_apts (defect_apts_id, defect_type, desgnr, reinf_method, reinf_status, sprvsr, zipcode, apts_id)
VALUES (1, '도면표현 미흡', '숨비/노드플랜,광장구조', '보완 완료', '기둥신설, 슬래브보완완료', '직접감독', '12345', 'APT1168048624120011000001'),
       (2, '배근도 인해 및 도면검토 부족', '엠디에이,태경구조', '보완 완료', '슬래브보완 완료', '직접감독', '12345', 'APT3315032510130550003901');

-- 아파트 상세정보
INSERT INTO detail_apts (detail_apts_id, electricity_safety_manager_assigned, emergency_license_plate_recognition, emergency_vehicle_accessible_gate, emergency_vehicle_instant_pass, ground_accessible_to_public, ground_ev_charger_installed, home_network, underground_accessible_to_public, underground_ev_charger_installed, annual_disinfection_frequency, basement_floor_count, cargo_elevator_count, cctv_count, cleaning_management_staff, disabled_elevator_count, electric_vehicle_count, electricity_supply_capacity, emergency_elevator_count, general_management_staff, ground_ev_charger_count, ground_ev_parking_spaces, ground_parking_spaces, max_floor_count, max_floor_count_building_register, number_of_buildings, number_of_private_rent_units, number_of_public_rent_units, number_of_rent_units, number_of_sale_units, number_of_units, other_elevator_count, passenger_cargo_elevator_count, passenger_elevator_count, security_management_staff, total_parking_spaces, total_vehicle_count, underground_ev_charger_count, underground_ev_parking_spaces, underground_parking_spaces, amenities, approval_date, building_structure, cleaning_management_contractor, cleaning_management_type, complex_type, constructor, corridor_type, developer, disinfection_management_contractor, disinfection_management_type, disinfection_method, electricity_contract_type_per_unit, elevator_management_type, emergency_vehicle_access_method, ev_charging_facilities_details, fire_receiver_type, food_waste_disposal_method, general_management_type, ground_access_end_time, ground_access_start_time, heating_type, housing_manager, housing_manager_business_registration_number, management_office_address, management_office_contact, management_office_fax, management_type, postal_code, registration_date, resident_facilities, road_address, sale_type, security_management_contractor, security_management_type, selected_gate_parking_control_system, underground_access_end_time, underground_access_start_time, water_supply_method, apts_id)
VALUES ('A10023132', 1, 1, 1, 1, 1, 0, 1, 1, 1, 4, 2, 0, 320, 6, 0, 32, 2350, 0, 6, 0, 0, 4, 15, 15, 8, 0, 199, 199, 398, 597, 0, 0, 16, 4, 723, 776, 37, 37, 719, '관리사무소, 보육시설, 문고, 주민공동시설, 어린이놀이터, 휴게시설, 커뮤니티공간, 자전거보관소', '20230613', '철근콘크리트구조', '(주)홈스웰', '위탁관리', '', '양우건설', '혼합식', '대한토지주택공사', '(주)한국종합방제', '위탁관리', '분무식', '단일계약', '위탁관리', '여', '◆1◆|지하|벽부형충전기|AC단상 5핀|완속|36|클린일렉스|,◆2◆|지하|스탠드형충전기|DC콤보 7핀|급속|1|클린일렉스|', 'GR형', '음식물쓰레기종량제', '위탁관리', '0시 0분', '0시 0분', '지역난방', '우리관리(주)', '1148123107', '서울 강남구 자곡로11길 11 (디아크리온강남)', '024452041', '024452043', '위탁관리', '06369', '20230710', '있음', '서울특별시 강남구 자곡로11길 11', '혼합', '(주)홈스웰', '위탁관리', '정문', '23시 59분', '시 분', '부스타방식', 'APT1168048624120011000001'),
       ('A41370508', 1, 0, 1, 1, 1, 1, 0, 1, 1, 12, 1, 0, 205, 12, 0, 5, 3600, 0, 10, 38, 38, 1020, 23, 23, 21, 0, 0, 0, 1638, 1638, 0, 0, 46, 14, 1679, 5, 4, 4, 659, '관리사무소, 노인정, 보육시설, 주민공동시설, 어린이놀이터, 자전거보관소', '20041115', '철근콘크리트구조', '청아시스템', '위탁관리', '아파트', '대한토지주택공사', '계단식', '대한토지주택공사', '주신환경', '위탁관리', '도포식,분무식', '단일계약', '위탁관리', '가능', '◆1◆|지하|스탠드형충전기|AC단상 5핀|급속|2|한국전력|,◆2◆|지하|스탠드형충전기|DC차데모 10핀|완속|2|한국전력|,◆3◆|지상|스탠드형충전기|AC3상 7핀|완속|38|에버온|', 'R형', '음식물쓰레기종량제', '위탁관리', '23시 59분', '23시 59분', '개별난방', '대원종합관리(주)', '1178106882', '경기도 파주시 후곡로 50 후곡마을4단지 관리사무소', '0319414157', '0319414159', '위탁관리', '10920', '20090728', '없음', '경기도 파주시 후곡로 50', '분양', '대한종합관리(주)', '위탁관리', '정문', '23시 58분', '23시 59분', '부스타방식', 'APT4148032061370050000001');

-- 관리비 상세정보
INSERT INTO mng_cost (mngcost_id, province, city_county_district, town_village, street, complex_name, occurrence_year_month, individual_usage_sum, heating_cost_common, heating_cost_individual, hot_water_cost_common, hot_water_cost_individual, gas_usage_cost_common, gas_usage_cost_individual, electricity_cost_common, electricity_cost_individual, water_cost_common, water_cost_individual, tvfee, sewage_fee, waste_fee, association_cost, building_insurance_fee, election_cost, etc, reserve_fund_monthly_charge, reserve_fund_monthly_expenditure, reserve_fund_total_accumulated, reserve_fund_accumulation_rate, miscellaneous_income_monthly_amount, resident_contribution_revenue, common_contribution_revenue, total_common_management_fee_sum, labor_cost, office_expenses, taxes_and_dues, clothing_cost, training_cost, vehicle_maintenance_cost, other_incidental_expenses, cleaning_cost, security_cost, disinfection_cost, elevator_maintenance_cost, intelligent_network_maintenance, repair_cost, facility_maintenance_cost, safety_inspection_cost, disaster_prevention_cost, management_commission_fee, detail_apts_id)
VALUES ('1', '서울특별시', '강남구', '', '자곡동', '디아크리온강남 아파트', 202401, 60456168, 26334870, 1187981, 369770, 234300, 190000, 0, 919679, 13916980, 14799360, 0, 1760000, 0, 0, 743228, 0, 0, 0, 111925736, 1369348, 29646750, 0, 22154860, 0, 736480, 16231360, 28490410, 0, 9702190, 0, 0, 736480, 1824900, 782955, 250000, 0, 0, 0, 0, 0, 173997, 0, 173997, 'A10023132'),
...
-- 공지사항 더미 데이터
INSERT INTO notices (notices_id, content, created_at, title, updated_at)
VALUES (1, '아파트 정보 조회 서비스가 런칭되었습니다.\n많은 이용 부탁드립니다. 감사합니다.', '2025-02-08 11:05:59.000000', '아파트 정보 조회 서비스 첫 서비스 시작..!(v1.0.0)', '2025-02-08 11:06:01.000000'),
       (2, '뉴스 데이터 및 아파트 상세 정보 부분 버그가 수정되었습니다.', '2025-02-10 11:07:36.000000', '서비스 업데이트(v1.0.1)', '2025-02-10 11:07:38.000000'),
       (3, '현재 뉴스 데이터가 일반 아파트 정보와 부실 아파트 정보가 구분되지 않는 버그가 발생하였습니다. 조속히 해결 하도록 하겠습니다.\n 감사합니다.', '2025-02-12 11:08:43.000000', '버그 안내', '2025-02-12 11:08:44.000000'),
       (4, '현재 사이트에 트래픽이 많이 발생함으로 불편을 야기할 수도 있습니다.', '2025-02-12 11:09:22.000000', '트래픽 안내', '2025-02-12 11:09:23.000000');
